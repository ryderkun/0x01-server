...
new({MinX, MinY, MaxX, MaxY}, SplitTimes) ->
    new(MinX, MinY, MaxX, MaxY, SplitTimes).
...
find({QuadMaps, _}, X1, Y1, X2, Y2) ->
    do_find(maps:get(1, QuadMaps), QuadMaps, X1, Y1, X2, Y2, []).

do_find(#quad{quardrants = {}, elements = Es}=Quad, _, X1, Y1, X2, Y2, Got) ->
    case quadrand_contains(Quad, X1, Y1, X2, Y2) of
        true -> Got ++ Es;
        false -> Got
    end;

do_find(#quad{quardrants = {Q1ID, Q2ID, Q3ID, Q4ID}}=Quad, QuadMaps, X1, Y1, X2, Y2, Got) ->
    case quadrand_contains(Quad, X1, Y1, X2, Y2) of
        true ->
            E1 = do_find(maps:get(Q1ID, QuadMaps), QuadMaps, X1, Y1, X2, Y2, Got),
            E2 = do_find(maps:get(Q2ID, QuadMaps), QuadMaps, X1, Y1, X2, Y2, E1),
            E3 = do_find(maps:get(Q3ID, QuadMaps), QuadMaps, X1, Y1, X2, Y2, E2),
            E4 = do_find(maps:get(Q4ID, QuadMaps), QuadMaps, X1, Y1, X2, Y2, E3),
            E4;
        false ->
            Got
    end.
