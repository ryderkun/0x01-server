...
%% Generated by gpb_compile version 3.18.4 on {{2015,6,15},{16,21,19}}
-module(protocol).
...
dfp_read_field_def_ProtocolConfig(<<16, Rest/binary>>,
				  Z1, Z2, F1, F2, F3, F4, F5) ->
    d_field_ProtocolConfig_map_minx(Rest, Z1, Z2, F1, F2,
				    F3, F4, F5);
dfp_read_field_def_ProtocolConfig(<<24, Rest/binary>>,
				  Z1, Z2, F1, F2, F3, F4, F5) ->
    d_field_ProtocolConfig_map_miny(Rest, Z1, Z2, F1, F2,
				    F3, F4, F5);
dfp_read_field_def_ProtocolConfig(<<32, Rest/binary>>,
				  Z1, Z2, F1, F2, F3, F4, F5) ->
    d_field_ProtocolConfig_map_maxx(Rest, Z1, Z2, F1, F2,
				    F3, F4, F5);
dfp_read_field_def_ProtocolConfig(<<40, Rest/binary>>,
				  Z1, Z2, F1, F2, F3, F4, F5) ->
    d_field_ProtocolConfig_map_maxy(Rest, Z1, Z2, F1, F2,
				    F3, F4, F5);
dfp_read_field_def_ProtocolConfig(<<>>, 0, 0, F1, F2,
				  F3, F4, F5) ->
    #'ProtocolConfig'{sync_interval = F1, map_minx = F2,
		      map_miny = F3, map_maxx = F4, map_maxy = F5};
...
dg_read_field_def_ProtocolConfig(<<0:1, X:7,
    ...
    case Key of
	  ...
	  d_field_ProtocolConfig_sync_interval(Rest, 0, 0, F1, F2,
					       F3, F4, F5);
      16 ->
	  d_field_ProtocolConfig_map_minx(Rest, 0, 0, F1, F2, F3,
					  F4, F5);
      24 ->
	  d_field_ProtocolConfig_map_miny(Rest, 0, 0, F1, F2, F3,
					  F4, F5);
      32 ->
	  d_field_ProtocolConfig_map_maxx(Rest, 0, 0, F1, F2, F3,
					  F4, F5);
      40 ->
	  d_field_ProtocolConfig_map_maxy(Rest, 0, 0, F1, F2, F3,
					  F4, F5);
      _ ->
	  case Key band 7 of
	    0 ->
		skip_varint_ProtocolConfig(Rest, 0, 0, F1, F2, F3, F4,
					   F5);
	    1 ->
		skip_64_ProtocolConfig(Rest, 0, 0, F1, F2, F3, F4, F5);
	    2 ->
		skip_length_delimited_ProtocolConfig(Rest, 0, 0, F1, F2,
						     F3, F4, F5);
	    5 ->
		skip_32_ProtocolConfig(Rest, 0, 0, F1, F2, F3, F4, F5)
	  end
    end;
dg_read_field_def_ProtocolConfig(<<>>, 0, 0, F1, F2, F3,
				 F4, F5) ->
    #'ProtocolConfig'{sync_interval = F1, map_minx = F2,
		      map_miny = F3, map_maxx = F4, map_maxy = F5}.
...
d_field_ProtocolConfig_map_minx(<<1:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, F2, F3, F4, F5)
    when N < 57 ->
    d_field_ProtocolConfig_map_minx(Rest, N + 7,
				    X bsl N + Acc, F1, F2, F3, F4, F5);
d_field_ProtocolConfig_map_minx(<<0:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, _, F3, F4, F5) ->
    <<NewFValue:32/signed-native>> = <<(X bsl N +
					  Acc):32/unsigned-native>>,
...
d_field_ProtocolConfig_map_miny(<<1:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, F2, F3, F4, F5)
    when N < 57 ->
    d_field_ProtocolConfig_map_miny(Rest, N + 7,
				    X bsl N + Acc, F1, F2, F3, F4, F5);
d_field_ProtocolConfig_map_miny(<<0:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, F2, _, F4, F5) ->
    <<NewFValue:32/signed-native>> = <<(X bsl N +
					  Acc):32/unsigned-native>>,
...
d_field_ProtocolConfig_map_maxx(<<1:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, F2, F3, F4, F5)
    when N < 57 ->
    d_field_ProtocolConfig_map_maxx(Rest, N + 7,
				    X bsl N + Acc, F1, F2, F3, F4, F5);
d_field_ProtocolConfig_map_maxx(<<0:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, F2, F3, _, F5) ->
    <<NewFValue:32/signed-native>> = <<(X bsl N +
					  Acc):32/unsigned-native>>,
...
d_field_ProtocolConfig_map_maxy(<<1:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, F2, F3, F4, F5)
    when N < 57 ->
    d_field_ProtocolConfig_map_maxy(Rest, N + 7,
				    X bsl N + Acc, F1, F2, F3, F4, F5);
d_field_ProtocolConfig_map_maxy(<<0:1, X:7,
				  Rest/binary>>,
				N, Acc, F1, F2, F3, F4, _) ->
    <<NewFValue:32/signed-native>> = <<(X bsl N +
					  Acc):32/unsigned-native>>,
...
skip_varint_ProtocolConfig(<<1:1, _:7, Rest/binary>>,
			   Z1, Z2, F1, F2, F3, F4, F5) ->
    skip_varint_ProtocolConfig(Rest, Z1, Z2, F1, F2, F3, F4,
			       F5);
...
merge_msg_ProtocolConfig(#'ProtocolConfig'{sync_interval
    ...
    #'ProtocolConfig'{sync_interval =
		      ...
		      map_minx =
			  if NFmap_minx =:= undefined -> PFmap_minx;
			     true -> NFmap_minx
			  end,
		      map_miny =
			  if NFmap_miny =:= undefined -> PFmap_miny;
			     true -> NFmap_miny
			  end,
		      map_maxx =
			  if NFmap_maxx =:= undefined -> PFmap_maxx;
			     true -> NFmap_maxx
			  end,
		      map_maxy =
			  if NFmap_maxy =:= undefined -> PFmap_maxy;
			     true -> NFmap_maxy
			  end}.
...
v_msg_ProtocolConfig(#'ProtocolConfig'{sync_interval =
    ...
    v_type_int32(F2, [map_minx | Path]),
    v_type_int32(F3, [map_miny | Path]),
    v_type_int32(F4, [map_maxx | Path]),
    v_type_int32(F5, [map_maxy | Path]),
    ok.
    ...
    [{{msg, 'ProtocolVector2'},
     ...
     {{msg, 'ProtocolConfig'},
      [#field{name = sync_interval, fnum = 1, rnum = 2,
       ...
       #field{name = map_minx, fnum = 2, rnum = 3,
	      type = int32, occurrence = required, opts = []},
       #field{name = map_miny, fnum = 3, rnum = 4,
	      type = int32, occurrence = required, opts = []},
       #field{name = map_maxx, fnum = 4, rnum = 5,
	      type = int32, occurrence = required, opts = []},
       #field{name = map_maxy, fnum = 5, rnum = 6,
	      type = int32, occurrence = required, opts = []}]},
...
get_msg_names() ->
    ['ProtocolVector2', 'ProtocolUnitCreate',
     'ProtocolUnitMove', 'ProtocolUnitEject', 'ProtocolDot',
     'ProtocolUnit', 'ProtocolSceneInit', 'ProtocolUnitAdd',
     'ProtocolSceneSync', 'ProtocolConfig',
