...
%% Generated by gpb_compile version 3.18.4 on {{2015,6,19},{0,42,57}}
-module(protocol).
...
e_msg_ProtocolUnitMove(#'ProtocolUnitMove'{towards =
					       F1},
		       Bin) ->
    e_mfield_ProtocolUnitMove_towards(F1,
				      <<Bin/binary, 10>>).
...
e_mfield_ProtocolUnitMove_towards(Msg, Bin) ->
    Bin2 = <<Bin/binary, 10>>,
    e_msg_ProtocolVector2(Msg, Bin2).
...
dfp_read_field_def_ProtocolUnitMove(<<10, Rest/binary>>,
				    Z1, Z2, F1) ->
    d_field_ProtocolUnitMove_towards(Rest, Z1, Z2, F1);
dfp_read_field_def_ProtocolUnitMove(<<>>, 0, 0, F1) ->
    #'ProtocolUnitMove'{towards = F1};
...
dg_read_field_def_ProtocolUnitMove(<<0:1, X:7,
    ...
    Key = X bsl N + Acc,
    case Key of
      10 -> d_field_ProtocolUnitMove_towards(Rest, 0, 0, F1);
      _ ->
	  case Key band 7 of
...
d_field_ProtocolUnitMove_towards(<<1:1, X:7,
				   Rest/binary>>,
				 N, Acc, F1)
    when N < 57 ->
    d_field_ProtocolUnitMove_towards(Rest, N + 7,
				     X bsl N + Acc, F1);
d_field_ProtocolUnitMove_towards(<<0:1, X:7,
				   Rest/binary>>,
				 N, Acc, F1) ->
    Len = X bsl N + Acc,
    <<Bs:Len/binary, Rest2/binary>> = Rest,
...
merge_msg_ProtocolUnitMove(#'ProtocolUnitMove'{towards =
						   PFtowards},
			   #'ProtocolUnitMove'{towards = NFtowards}) ->
    #'ProtocolUnitMove'{towards =
...
v_msg_ProtocolUnitMove(#'ProtocolUnitMove'{towards =
					       F1},
		       Path) ->
    v_msg_ProtocolVector2(F1, [towards | Path]), ok.
...
get_msg_defs() ->
    [{{msg, 'ProtocolVector2'},
     ...
     {{msg, 'ProtocolUnitMove'},
      [#field{name = towards, fnum = 1, rnum = 2,
	      type = {msg, 'ProtocolVector2'}, occurrence = required,
	      opts = []}]},
     {{msg, 'ProtocolDot'},
